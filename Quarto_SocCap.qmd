...
```{r echo=FALSE}
# Mapping Margin Votes 2016
map2016 <- ggplot(data = counties) +
  geom_sf(aes(fill = MAR_T_new, geometry = geometry), color = "transparent", size = 0.5) +
  scale_fill_viridis_c(name = "US Counties", option = "C") +
  labs(title = "Trump Margin 2016") +
  theme_map()

# Mapping Margin Votes 2020
map2020 <- ggplot(data = counties) +
  geom_sf(aes(fill = MAR_T_20_new, geometry = geometry), color = "transparent", size = 0.5) +
  scale_fill_viridis_c(name = "US Counties", option = "C") +
